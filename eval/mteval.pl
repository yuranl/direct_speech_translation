...
#
# version 13a
#    * modified by HE Zhongjun, read sgml file in which seg id need NOT enclosed with double-quotes or signle-quotes. 
...
print "srcid: $src_id; refid: $ref_id; ";
print "tstid: $tst_id\n";
...
sub get_source_info
{
	...
	else
	{
        ...
        while (($tag, $span, $seg_data) = extract_sgml_tag_and_span ("Seg", $seg_data)) {
            ($eval_docs{$doc}{SEGS}{$jseg++}) = &{ $TOKENIZATION }( $span );
        }
        die "\n\nFATAL INPUT ERROR:  no segments in document '$doc' in file '$file'\n\n"
            if $jseg == 0;
		}
...
sub get_MT_data
{
	...
	else
	{
		...
		#get tag info
		#($tag, $span, $data) = extract_sgml_tag_and_span ($set_tag, $data);
		#print "Set tag: $set_tag\n";
		#print "Getting tag info for: $tag\n";
		while (($tag, $span, $data) = extract_sgml_tag_and_span ($set_tag, $data))
		{
			die "\n\nFATAL INPUT ERROR:  no tag attribute '$name' in file '$file'\n\n"
				unless ($id) = extract_sgml_tag_attribute ($name="SetID", $tag);
			#print "id set: $id for $set_tag\n";
			die "\n\nFATAL INPUT ERROR:  no tag attribute '$name' in file '$file'\n\n"
				unless ($src) = extract_sgml_tag_attribute ($name="SrcLang", $tag);
			die "\n\nFATAL INPUT ERROR:  $name ('$src') in file '$file' inconsistent\n"
			...
			while (($tag, $span, $mtdata) = extract_sgml_tag_and_span ("Doc", $mtdata))
			{
		     ...
		     my $jseg=0, my $seg_data = $span;
         while (($tag, $span, $seg_data) = extract_sgml_tag_and_span ("Seg", $seg_data)) {
               ($docs->{$sys}{$doc}{SEGS}{$jseg++}) = &{ $TOKENIZATION }( $span );
            }
            die "\n\nFATAL INPUT ERROR:  no segments in document '$doc' in file '$file'\n\n"
                if $jseg == 0;
...
#   foreach $doc (sort { $eval_docs{$a} <=> $eval_docs{$b} } keys %eval_docs)
	foreach $doc (sort keys %eval_docs)
	{
		...
		{
			my %DOCmt = ();
			my ($docScore, $BP) = &{$BLEU_SCORE}( $ref_length, $match_cnt, $tst_cnt, $sys, \%DOCmt );
			$overallScore->{ $sys }{ 'documents' }{ $doc }{ 'score' } = $docScore;
			if ( $detail >= 1 )
			{
				printf "$method score using   4-grams = %.4f BP = %.4f for system \"$sys\" on document \"$doc\" (%d segments, %d words)\n",
					$docScore, $BP, scalar keys %{$tst_data{$sys}{$doc}{SEGS}}, $tst_cnt->[1];
			}
		}
...
#score each segment
	foreach my $seg ( sort{ $a <=> $b } keys( %{$tst_data{$sys}{$doc}{SEGS}} ) )
	{
		...
		{
			my %DOCmt = ();
			my ($segScore, $BP) = &{$BLEU_SCORE}($ref_length, $match_cnt, $tst_cnt, $sys, %DOCmt);
...
sub bleu_score
{
	...
	my $exp_len_score = 0;
    my $BP = 0.0;
    $BP = min (0, 1 - $ref_length / $tst_ngrams->[ 1 ] ) if ( $tst_ngrams->[ 1 ] != 0 );
